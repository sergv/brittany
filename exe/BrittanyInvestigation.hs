...
module BrittanyInvestigation (main) where
...
main = do
  ...
  for_ args $ \path -> do
    ...
    -- args     <- getArgs
    -- parseModuleFromString args fp dynCheck (T.unpack contents)
    res <- parseModuleFromString [] path (\_ -> pure $ Right ()) $ T.unpack contents
    case res of
      Left err                      -> die $ "Error: " ++ err
      Right (x :: ParsedSource, ()) ->
        putStrLn $ renderString $ "parsed module:" ## pretty (makeDeltaAst x)
    pure ()
